...
# took out -nc because in combination with -O it causes a non-zero return value even on success
wget https://github.com/Itseez/opencv_contrib/archive/$VERSION.tar.gz -O opencv_contrib-$VERSION.tar.gz

tar xfzv $VERSION.tar.gz
tar xfzv opencv_contrib-$VERSION.tar.gz

CONTRIB_DIR=`pwd`/opencv_contrib-$VERSION
