...
bool
Hungarian::find_uncovered_in_matrix(double item, unsigned int &row, unsigned int &col) const {
...
bool
Hungarian::pair_in_list(const std::pair<int,int> &needle, const std::list<std::pair<int,int> > &haystack) {
...
void Hungarian::diag(bool status){
  isDiag = status;
}
